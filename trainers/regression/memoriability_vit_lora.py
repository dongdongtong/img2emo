...
class MemModel(nn.Module):
    def __init__(self, cfg, ):
        ...
        if cfg.MODEL.NAME == "maxvit_t":
            ...
            lora_reduction = cfg.TRAINER.MEMVITLORA.LORA_REDUCTION
            ...
            self.class_head = nn.Sequential(
                    ...
                    nn.ReLU(),
                    nn.Dropout(cfg.TRAINER.MEMVITLORA.HEAD_DROPRATE),
                    nn.Linear(256, 1)
                )
        else:
...
class MemVitLora(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.MEMVITLORA.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.MEMVITLORA.PREC == "amp" or cfg.TRAINER.MEMVITLORA.PREC == "fp32":
            self.model.float()
        else:
            self.model.half()
        ...
        self.scaler = GradScaler() if cfg.TRAINER.MEMVITLORA.PREC == "amp" else None
        ...
        self.srcc_criterion = SpearmanLoss(differentiable=True, temperature=cfg.TRAINER.MEMVITLORA.SRCC_TEMP)
    ...
    def get_loss(self, batch_x):
        ...
        if self.cfg.TRAINER.MEMVITLORA.METHOD == 1:
            loss_dict = self.mse_loss(images, labels)
        elif self.cfg.TRAINER.MEMVITLORA.METHOD == 2:
            loss_dict = self.mse_srcc_loss(images, labels)
        elif self.cfg.TRAINER.MEMVITLORA.METHOD == 3:
            loss_dict = self.mse_ccc_srcc_loss(images, labels)
        else:
            raise ValueError(f"Unknown method: {self.cfg.TRAINER.MEMVITLORA.METHOD}")
    ...
    def forward_backward(self, batch_x):
        prec = self.cfg.TRAINER.MEMVITLORA.PREC
    ...
    def model_backward_and_update(self, loss, meta_loss_dict, names=None, grad_record=False, grad_clip=False, grad_tag="g"):
        ...
        if self.cfg.TRAINER.MEMVITLORA.PREC == "amp":
