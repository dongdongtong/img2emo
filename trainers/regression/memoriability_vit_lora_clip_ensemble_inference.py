...
class MemModel(nn.Module):
    def __init__(self, cfg, device):
        ...
        if cfg.MODEL.NAME == "maxvit_t":
            ...
            lora_reduction = cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.LORA_REDUCTION
            ...
            self.class_head = nn.Sequential(
                    ...
                    nn.ReLU(),
                    nn.Dropout(cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.HEAD_DROPRATE),
                    nn.Linear(256, 1)
                )
        else:
...
class EnsembleMemCLIPVitLora(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        # this is a list
        INITI_WEIGHTS = cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.INIT_WEIGHTS
        
        models = []
        if INITI_WEIGHTS is not None:
            
            for init_weights in INITI_WEIGHTS:
                model = MemModel(cfg, self.device)
                model.to(self.device)

                state_dict = load_checkpoint(init_weights)
                
                load_pretrained_model_lora_classhead(model, state_dict)
        ...
        if cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.PREC == "amp" or cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.PREC == "fp32":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.PREC == "amp" else None
        ...
        self.srcc_criterion = SpearmanLoss(differentiable=True, temperature=cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.SRCC_TEMP)
    ...
    def get_loss(self, batch_x):
        ...
        if self.cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.METHOD == 1:
            loss_dict = self.mse_loss(images, labels)
        elif self.cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.METHOD == 2:
            loss_dict = self.mse_srcc_loss(images, labels)
        elif self.cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.METHOD == 3:
            loss_dict = self.mse_ccc_srcc_loss(images, labels)
        else:
            raise ValueError(f"Unknown method: {self.cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.METHOD}")
    ...
    def forward_backward(self, batch_x):
        prec = self.cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.PREC
    ...
    def model_backward_and_update(self, loss, meta_loss_dict, names=None, grad_record=False, grad_clip=False, grad_tag="g"):
        ...
        if self.cfg.TRAINER.ENSEMBLEMEMCLIPVITLORA.PREC == "amp":
