...
class LNSIM(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        label_data = label_data["subject_hrs1"][fold]   # for LNSIM, we have multiple group of labels, which is generated by different raters
        
        
        train_val_idxs, test_idxs = train_test_split(range(len(image_paths)), test_size=0.2, random_state=42)
        train_idxs, val_idxs = train_test_split(train_val_idxs, test_size=0.2, random_state=42)
        train_image_paths, y_train = zip(*[(image_paths[i], label_data[i]) for i in train_idxs])
        val_image_paths, y_val = zip(*[(image_paths[i], label_data[i]) for i in val_idxs])
        test_image_paths, y_test = zip(*[(image_paths[i], label_data[i]) for i in test_idxs])
