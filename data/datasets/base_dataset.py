...
class MultiLabelDatum(Datum):
    def __init__(self, impath="", label: Sequence[float] = None, domain=0, classname=""):
        super().__init__(impath, label, domain, classname)
        assert isinstance(label, (list, tuple))
        assert all(isinstance(l, (int, float)) for l in label)
...
class ContinuousLabelDatum(Datum):
    """Data instance which defines the basic attributes, note our label are a float value.
    ...
    def __init__(self, impath="", label: float=0.0, domain=0, classname=""):
        assert isinstance(impath, str)
        assert check_isfile(impath)
